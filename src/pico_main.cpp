...
void update(uint32_t) {
  ...
  persist_best();
#endif
  if (game.mode == rally::Mode::Racing && now - last_sound_us >= 50000) {
    last_sound_us = now;
    if (game.impact > .3f)
      ...
      play(voice(0, 0, 80, 10, 0, 0, 0, 12, 12), 65 + int(game.speed * 8), 75, 22);
  }
  if (stats.tick_us && game.mode == rally::Mode::Racing) {
    ++frames;
    total_frame += stats.tick_us;
    if (stats.tick_us > 33334)
      ++slow_frames;
  ...
  if (now - last_report >= 2000000 && stdio_usb_connected()) {
    last_report = now;
