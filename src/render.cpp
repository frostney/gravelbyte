...
Renderer::Renderer() {
  for (int i = 0; i < int(ridge_heights.size()); ++i)
    ridge_heights[i] = uint8_t(
        29 + int(3 * std::sin(i * 6.2831853f * 6 / 512) + 2 * std::sin(i * 6.2831853f * 14 / 512)));
}
...
void Renderer::project(CameraVertex &v) {
  if (v.z < 42)
    return;
  v.sx = int16_t(std::clamp<int32_t>(60 + 85 * v.x / v.z, -2000, 2000));
  v.sy = int16_t(std::clamp<int32_t>(53 - 85 * v.y / v.z, -2000, 2000));
  v.inverse_z = uint16_t(2097152 / v.z);
}
Renderer::CameraVertex Renderer::transform(Vec world) {
  ...
  const int32_t wx = int32_t(world.x * 64), wy = int32_t(world.y * 64), wz = int32_t(world.z * 64);
  uint32_t hash = uint32_t(wx) * 73856093u ^ uint32_t(wy) * 19349663u ^ uint32_t(wz) * 83492791u;
  auto &cached = vertex_cache[(hash ^ (hash >> 16)) & 511u];
  ...
  const int32_t forward = (x * sine + z * cosine) >> 14;
  CameraVertex v;
  v.x = (x * cosine - z * sine) >> 14;
  v.y = (y * 16066 + forward * 3213) >> 14;
  v.z = (forward * 16066 - y * 3213) >> 14;
  project(v);
...
void Renderer::triangle(Vec a, Vec b, Vec c, uint16_t col) {
  ...
  if (input[0].z > 170 * 64 && input[1].z > 170 * 64 && input[2].z > 170 * 64)
    return;
  // Clip against near plane before perspective divide (no road popping at camera).
  int count = 0;
  for (int i = 0; i < 3; ++i) {
    CameraVertex p = input[i], q = input[(i + 1) % 3];
    bool p_in = p.z >= 42, q_in = q.z >= 42;
    if (p_in)
      clipped[count++] = p;
    if (p_in != q_in) {
      CameraVertex edge;
      edge.x = p.x + (q.x - p.x) * (42 - p.z) / (q.z - p.z);
      edge.y = p.y + (q.y - p.y) * (42 - p.z) / (q.z - p.z);
      edge.z = 42;
      project(edge);
      clipped[count++] = edge;
    }
...
void Renderer::render(const Game &g, uint16_t *target, int fps, bool diagnostics) {
  ...
  cam_sin = std::sin(g.camera_yaw);
  cam_cos = std::cos(g.camera_yaw);
  camera = g.car + Vec{-cam_sin * 7.5f, 3.6f, -cam_cos * 7.5f};
  camera.y = g.camera_height + 3.6f;
  camera_x = int32_t(camera.x * 64);
  camera_y = int32_t(camera.y * 64);
  camera_z = int32_t(camera.z * 64);
  sine = int32_t(cam_sin * 16384);
  cosine = int32_t(cam_cos * 16384);
  if (++render_frame == 0) {
  ...
  for (int x = 0; x < W; ++x) {
    int phase = x + int(g.camera_yaw * 28);
    int ridge = ridge_heights[unsigned(phase) & 511u];
    rect(x, ridge - 5, 1, 29, color(8, 9, 10));
    rect(x, ridge + 4 + (ridge_heights[(unsigned(phase) + 143) & 511u] - 29) * 2, 1, 28,
         color(7, 8, 8));
  }
  rect(0, 52, W, H - 52, color(6, 7, 5));
  if (g.selected_track == 1) {
    rect(0, 0, W, 25, color(9, 12, 13));
    rect(0, 52, W, H - 52, color(9, 9, 4));
  }
  ...
  const int first = std::max(0, g.segment - 5), last = std::min(NodeCount - 1, g.segment + 31);
  for (int i = first; !showroom && i < last; ++i) {
    ...
    for (int side = 0; side < 2; ++side) {
      ...
      ground_triangle(a[far], b[far], b[bank], shade(grass, -1));
      ground_triangle(a[far], b[bank], a[bank], grass);
      ground_triangle(a[bank], b[bank], b[verge], shade(grass, i % 3 == 0 ? 1 : 0));
      ground_triangle(a[bank], b[verge], a[verge], shade(grass, -1));
  ...
  if (g.mode == Mode::Title) {
    rect(4, 12, 112, 29, dark);
    rect(4, 12, 3, 29, yellow);
    centered(18, "GRAVELBYTE", white, 2);
    centered(33, "SMALL CAR. BIG RALLY.", yellow);
    rect(7, 91, 106, 23, dark);
    std::snprintf(buffer, sizeof(buffer), "%s  DRIVE", confirm);
    centered(97, buffer, white);
    centered(106, "THREE ROADS TO MASTER", yellow);
    return;
  }
  if (g.mode == Mode::CarSelect) {
    ...
    for (int row = 0; row < 3; ++row) {
      text(15, 84 + row * 8, labels[row], white);
      for (int k = 0; k < 5; ++k)
        rect(53 + k * 10, 84 + row * 8, 8, 4, k < values[row] ? yellow : color(4, 5, 5));
    }
  ...
  if (g.mode == Mode::Finished) {
    rect(5, 22, 110, 91, dark);
    centered(27, "STAGE CLEAR", yellow, 2);
    time_text(buffer, sizeof(buffer), g.elapsed);
    centered(41, buffer, white, 2);
    centered(54,
             g.elapsed < g.previous_best ? "TARGET BEATEN"
                                         : (g.new_record ? "PERSONAL BEST" : "FINISH TIME"),
             yellow);
    for (int i = 0; i < SectorCount; ++i) {
      std::snprintf(buffer, sizeof(buffer), "%d %+.2f SEC", i + 1,
                    g.splits[i] - g.reference_splits[i]);
      centered(63 + i * 7, buffer,
               g.splits[i] <= g.reference_splits[i] ? color(6, 15, 7) : color(15, 6, 4));
    }
    std::snprintf(buffer, sizeof(buffer), "%s RETRY  %s SELECT", confirm, back);
    centered(103, buffer, white);
  }
  if (diagnostics) {
