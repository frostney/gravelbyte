...
for (const [width, height, touch] of [
  ...
  test(`game and instructions fit without scrolling at ${width}x${height}`, async ({ browser }) => {
    ...
    await expect(page.getByRole('link', { name: 'Download for PicoSystem' })).toHaveAttribute(
      'href',
      /assets\/[0-9a-f]{40}\/gravelbyte\.uf2$/,
    );
    await context.close();
  });
}
...
for (const asset of ['gravelbyte.js', 'gravelbyte.wasm', 'app.js']) {
  test(`missing ${asset} reports a load error instead of hanging`, async ({ page }) => {
    await page.route(`**/${asset}`, (route) => route.fulfill({ status: 404, body: 'missing' }));
    await page.goto('/');
    await expect(page.locator('#load-status')).toContainText('could not load');
    await expect(page.locator('#play')).toBeDisabled();
  });
}
test('mixed release executable is rejected before play', async ({ page }) => {
...
test('corrupt and unavailable storage preserve playable controls', async ({ page }) => {
...
test('accessible menus announce choices, refuse locked stages and start an unlocked race', async ({
  ...
  await play(page);
  const chooseCar = page.getByRole('button', { name: 'Choose car', exact: true });
  await chooseCar.focus();
  await chooseCar.press('Enter');
  await expect(page.locator('#game-status')).toContainText('TORR, STANDARD');
  const next = page.getByRole('button', { name: 'Next car', exact: true });
  await next.focus();
  await next.press('Enter');
  await expect(page.locator('#game-status')).toContainText('KITE, EXPERT');
  const choose = page.getByRole('button', { name: 'Choose track', exact: true });
  await choose.focus();
  await choose.press('Enter');
  await expect(page.locator('#game-status')).toContainText('BRACKEN RIDGE. Unlocked');
  const nextTrack = page.getByRole('button', { name: 'Next track', exact: true });
...
test('leaving the game pauses and clears held throttle', async ({ page }) => {
  await play(page);
  await press(page, 'Enter');
  await press(page, 'Enter');
  await press(page, 'Enter');
...
test('static discovery metadata and download survive disabled JavaScript', async ({ browser }) => {
  ...
  await expect(page.getByRole('link', { name: 'Download for PicoSystem' })).toBeVisible();
  await expect(
    page.getByRole('heading', { name: 'Gravelbyte', exact: true, level: 1 }),
  ).toHaveCount(1);
  await context.close();
});
